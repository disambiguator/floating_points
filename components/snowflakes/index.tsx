...
const Shaders = React.memo(function Shader() {
  ...
  const onPointerDown = ({ uv }: ThreeEvent<PointerEvent>) => {
    ...
    const d1 = uv.distanceTo(ref.current!.uniforms.p1.value);
    const d2 = uv.distanceTo(ref.current!.uniforms.p2.value);
  ...
  const onPointerMove = ({ uv }: ThreeEvent<PointerEvent>) => {
    if (mouseDown.current !== null && uv) {
      uv.multiplyScalar(2).subScalar(1);
      kaleid(uv);
      ref.current!.uniforms[mouseDown.current].value = uv;
    }
  };
  ...
  const onPointerEnter = () => {
    ref.current!.uniforms.hover.value = true;
    animate.current = clock.elapsedTime;
  };

  const onPointerLeave = () => {
    ref.current!.uniforms.hover.value = false;
    animate.current = clock.elapsedTime;
  };

  useFrame(() => {
    if (animate.current !== null) {
      ref.current!.uniforms.time.value = ref.current!.uniforms.hover.value
        ? clock.elapsedTime - animate.current
        : 0.2 - (clock.elapsedTime - animate.current);
      if (
        ref.current!.uniforms.time.value > 1 ||
