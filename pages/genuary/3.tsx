...
const _shader = {
  ...
  void main()
  {
      ...
      // vec2 moonPos = vec2(0.3,0.5);
      vec2 moonPos = mod(vec2(time/5.,0.5),1.0);

      float moonDistance = length(moonPos - sunPos);
      float distance = length(p - sunPos);
      float moonRadius = 0.1;

      vec2 v = sunPos - p;
      vec2 u = sunPos - moonPos;

      // vec2 projection = dot(u, normalize(v)) * normalize(v);
      vec2 rejection = u - dot(u, v) / dot(v,v) * v;

      float isMoon = circ(p - moonPos, moonRadius);
      if(isMoon > 0.) {
        gl_FragColor = vec4(vec3(0.2,0.2,0.2), 1.0);
      } else if(
        distance > moonDistance + moonRadius
        // && acos(dot(normalize(moonPos - sunPos), normalize(p - sunPos))) < (moonRadius * 2.)
        &&
         length(rejection) < moonRadius
        ) {
        gl_FragColor = vec4(vec3(1.,0.,0.), 1.0);
      } else {
        float sunDistance = 0.8 - length(p - sunPos);
        vec3 sun = vec3(1.,1.,0.) * sunDistance * sunDistance;
