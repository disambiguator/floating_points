...
import fragmentShader from 'lib/shaders/rayworld.frag';
...
const Bars = React.memo(function Bars() {
  ...
  useEffect(() => {
    const update = () => {
      ...
      // } else {
      shader.uniforms.ta.value.set(0, 0, -1).applyQuaternion(camera.quaternion);
      console.log(shader.uniforms.ta.value);
      // }
    };
  ...
  return (
    <>
      ...
      <pointLight position={[210, 210, 0]} intensity={40000} />
...
export const rayworldConfig = {
  Contents: Bars,
  name: 'rayworld',
  controls: (
    <FlyControls makeDefault movementSpeed={50} rollSpeed={1} dragToLook />
  ),
